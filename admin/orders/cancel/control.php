...
$full_order  = count_open_order($search_query, $sort_by, 'Cancelled', $query_per_page);
$total_query = $full_order['total_query'];
$total_page  = $full_order['total_page']; // RESULT


/* --- CALL FUNCTION --- */
$listing_order = get_open_order($search_query, $sort_by, $first_record, $query_per_page, 'Cancelled');
...
/* --- STORED VALUE --- */
echo "<input type=\"hidden\" name=\"url\" id=\"url\" class=\"hidden\" value=\"http://".$_SERVER['HTTP_HOST'].get_dirname($_SERVER['PHP_SELF'])."/order-cancelled-view\">\n";
...
/* --- ACTION CONTROL --- */
if(isset($_POST['index_cancel_order'])){
