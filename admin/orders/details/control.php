...
function flag_paid($total, $confirm){
   ...
   if($flag_paid > 0){
      $print = "<p class=\"help-block help-danger\">Underpaid IDR : ".price($flag_paid)."</p>";
   }else{
      $print = "<p class=\"help-block help-success\">Overpaid IDR : ".price($flag_paid)."</p>";
   }
...
$info     = get_info();
$country  = get_country();
$province = get_province($detail['order_shipping_country']);
$city     = get_city($detail['order_shipping_country'], $detail['order_shipping_province']);
...
if(isset($_POST['btn-edit-order'])){
   ...
   */
   ...
   $courier_rate  = get_shipping_rate($courier['courier_id'], $ship_province, $ship_city);
   $data_shipping = ceil($_POST['hidden_total_weight'] / $courier_rate['courier_weight']) * $courier_rate['courier_rate'];
   
   $data_total    = $data_purchase_amount + $data_shipping;
	 ...
	 // if(empty($_POST['chk_qty'])){
	  ...
	  //}else{	    
		 ...
		 foreach($items as $items){
			...
			update_order_qty($data_qty, $items);
		 }
		 ...
		 $total_shipping = ceil($total_temp_amount / $courier_rate['courier_weight']) * $courier_rate['courier_rate'];
		 
		 update_add_header($total_purchase_item, $total_shipping, ($total_purchase_item + $total_shopping), $detail['order_id']);
		 */
...
if(isset($_POST['btn-order-detailing'])){
   ...
   /* --- DEFINED VARIABLE --- */
   $post_order_id     = $_POST['hidden_order_id'];
   
   if(!empty($_POST['modal_confirm_method'])){
      $confirm_method = $_POST['modal_confirm_method'];
   }else{
	   
      if(!empty($_POST['hidden_order_confirm_bank'])){
         $confirm_method = $_POST['hidden_order_confirm_bank'];
	  }else{
         $confirm_method = $detail['order_payment_method'];
	  }
	  
   }
   
   if(!empty($_POST['modal_confirm_name'])){
      $confirm_name = $_POST['modal_confirm_name'];
   }else{
	   
      if(!empty($_POST['hidden_order_confirm_name'])){
         $confirm_name = $_POST['hidden_order_confirm_name'];
	  }else{
         $confirm_name = $detail['order_billing_fullname'];
	  }
	  
   }
   
   if(!empty($_POST['modal_confirm_amount'])){
      $post_order_amount = $_POST['modal_confirm_amount'];
   }else{
	   
      if(!empty($_POST['hidden_order_confirm_amount'])){
         $post_order_amount = $_POST['hidden_order_confirm_amount'];
	  }else{
         $post_order_amount = $detail['order_total_amount'];
      }
   ...
   if($_POST['btn-order-detailing'] == "Mark as Paid"){
      db_update_mark_as_paid($post_order_id, $post_order_amount, $confirm_name, $confirm_method);
...
if(isset($_POST['btn-order-confirm'])){
   ...
   if($_POST['btn-order-confirm'] == "Confirm"){ 
	  ...
	  if(!empty($notify)){
		 ...
		 $mail_item      = detail_order_item($updated_detail['order_id']);
        ...
        <tr>
          <td>
            <table width="600" border="0" cellspacing="0" cellpadding="0" align="center" style="overflow: hidden; background: #000">
              <tr>
                <td style="padding-left: 15px; padding-top: 10px; padding-bottom: 10px">
                  <img src="'.$prefix_url.'static/thimthumb.php?src='.$general['logo'].'&h=40&q=80" height="50">
                </td>
              </tr>
            </table>
          </td>
                ...
                <table width="540" border="0" cellspacing="0" cellpadding="0" align="center" style="font-size: 11px; border:">
				  ...
				  foreach($mail_item as $item){
					 ...
					 $mail_body .='<tr>
                      ...
                      <td style="border-bottom: 1px solid #ccc; text-align: center">'.price($item['item_quantity']).'</td>
                ...
                <table width="540" border="0" cellspacing="0" cellpadding="0" align="center" style="font-size: 11px; line-height: 16px; padding-top: 7px">
                  <tbody>
                    ...
                    <tr>
                      ...
                      <td style="padding-bottom: 5px; padding-right: 5px; text-align: right">IDR '.price($detail['order_shipping_amount']).'</td>
                    </tr>
                    <tr>
                      <td style="border-top: 1px solid #ccc; padding: 7px 0 5px 280px">TOTAL</td>
                      <td style="border-top: 1px solid #ccc; padding-right: 5px; font-size: 14px; text-align: right"><b>IDR '.price($detail['order_total_amount']).'</b></td>
                    </tr>
                  <tbody>
                </table>
              </td>
