...
<form method="post">
            ...
            <div class="subnav">
              <div class="container clearfix">
                <h1><span class="glyphicon glyphicon-usd"></span> &nbsp; Sale</h1>
                ...
                <div class="btn-placeholder">
                  <input type="submit" class="btn btn-danger btn-sm" value="Remove All Discounts" name="btn_sale_add">
                  <input type="button" class="btn btn-success btn-sm" value="Apply Discount" onClick="validateSale()">
                  <input type="submit" class="btn btn-success btn-sm hidden" value="Apply Discount" id="btn_submit_sale" name="btn_sale_add">
                </div>
              </div>
            </div>
            ...
            <div class="container main">
              ...
              <div class="box row">
                <div class="desc col-xs-3">
                  <h3>Sale Details</h3>
                  <p>Manage sale type, discount amount, and date range.</p>
                </div>
                <div class="content col-xs-9">
                  <ul class="form-set">
                    <li class="form-group row">
                      <label class="col-xs-3" for="category">Sale type <span>*</span></label>
                      <div class="col-xs-9">
                        <select class="form-control" id="sale_option" name="category" onchange="textIDR()">
                  ...
                  <table class="table">
                    <thead>
                      <tr class="headings">
                        ...
                        <th class="sort" width="30%" onclick="sortBy('promo_value')">Sale <?php echo $arr_promo_value;?></th>
                      </tr>
                    ...
                    <tbody>
                      ...
                      <tr class="<?php echo $tr_class;?>" id="<?php echo "row_".$row?>" onclick="selectRow('<?php echo $row;?>')">
                        <td><input type="checkbox" name="type_id[]" value="<?php echo $sale['type_id'];?>" id="<?php echo "check_".$row?>" onmouseover="downCheck()" onmouseout="upCheck()" onclick="selectRowCheck('<?php echo $row;?>')"></td>
                        ...
                        if(!empty($promotions['promo_item_id'])){
						   ...
						   }else if(date('Y-m-d') >= $promotions['promo_end_datetime']){
                        ...
                        <div id="ajax_remove_wrapper_<?php echo $promotions['promo_item_id']?>">
                           <b><?php echo $suffix;?> OFF</b>
                           <p style="color: #999"><?php echo format_date_min($promotions['promo_start_datetime'])." - ".format_date_min($promotions['promo_end_datetime']);?></p>
                           <p style="color: #cc0000"><?php echo $days_left;?></p>
                           <div class="btn btn-danger btn-sm" style="position: absolute; right: 5px; top: 20px" onclick="ajxRemoveSale(<?php echo $promotions['promo_item_id']?>)">Remove</div>
                        </div>
...
function ajxRemoveSale(i){
   ...
   var ajx = $.ajax({
              type: "POST",
        url: "<?php echo $prefix_url?>custom/promotions/sale/ajax_remove_sale.php",
        data: {itemID:itemID},
