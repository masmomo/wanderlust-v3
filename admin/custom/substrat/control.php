...
$pgdata = page_init($equal_search, $default_sort_by, $null_search); // static/general.php
...
// STORED VALUE
echo "<input type=\"hidden\" name=\"url\" id=\"url\" class=\"hidden\" value=\"http://".$_SERVER['HTTP_HOST'].get_dirname($_SERVER['PHP_SELF'])."/sale-view\">\n";
...
if(isset($_POST['btn_filter_add'])){
   
   if($_POST['btn_filter_add'] == "Apply Discount"){
      //DEFINED VALUE 
      $promo_id      = $_POST['filter_id'];
      $promo_type_id = $_POST['type_id'];
   
      foreach($promo_type_id as $promo_type_id){
         updateSale($promo_id, $promo_type_id);
      }
	  
	  $_SESSION['alert'] = "success";
	  $_SESSION['msg']   = "Successfully added promo(s)";
	  
   }else if($_POST['btn_filter_add'] == "Remove All Filter"){
      deleteAllSale();
	  
	  $_SESSION['alert'] = "success";
	  $_SESSION['msg']   = "Successfully deleted all promo(s)";
   }
