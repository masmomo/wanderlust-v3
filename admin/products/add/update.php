	...
	*/
	...
	if ($product_id==''){
		...
		$sql = "INSERT INTO tbl_product
		              (product_category, product_name, product_size_type_id, product_date_added, product_order, product_alias, page_title, page_description) 
		...
		$get_id = mysql_query("SELECT * from tbl_product
		                       WHERE product_category = '$product_category' AND 
	...
	//type image
	if ($_FILES["type_image"]["tmp_name"][$i]!=null){
		
		$file_type = substr($_FILES["type_image"]["name"][$i],-4);
		$file_name = substr($_FILES["type_image"]["name"][$i], 0, -4);
		
		$tmp_type  = substr($_FILES["type_image"]["tmp_name"][$i],-4);
		$tmp_name  = substr($_FILES["type_image"]["tmp_name"][$i], 0, -4);

		$tmp_name = $_FILES["type_image"]["tmp_name"][$i];
		$name     = cleanurl($type_name[$i])."_".$date_only."_".cleanurl($file_name).$file_type;
		$error    = $_FILES["type_image"]["error"][$i];
		//$link = $_POST["type_image"mage_counter];
	
		if ($error == 0){
		   move_uploaded_file($tmp_name,"../files/uploads/color_image/$name");
		   $img_src="files/uploads/color_image/$name";
		}
		...
		$color_id_ = $color_id[$i];
		$img_src=$_POST["color_image_".$color_id_];
	}
	
	mysql_query("UPDATE tbl_product_type
				 SET type_image = '$img_src'
				 WHERE type_id = '$type_id_'",$conn);
