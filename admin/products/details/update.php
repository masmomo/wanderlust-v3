	   ...
	   if ($type_delete[$i]=='1'){		
		  ...
		  if ($type_id[$i]==''){
			...
			$sql = "INSERT INTO tbl_product_type(type_code, type_name, type_price, type_description, color_id, type_weight, product_id, type_order, page_title, page_description)
			        VALUES ('$type_code[$i]', '$type_name[$i]', '$type_price[$i]', '".escape_quote($type_description[$i])."', '$color_id[$i]', '$type_weight[$i]', '$product_id', '$counter', '$page_title', '$page_description')
			";
		...
		//!edit product type
			...
			$sql = "UPDATE tbl_product_type SET type_code = '$type_code[$i]', 
												...
												type_price = '$type_price[$i]', 
												type_description = '".escape_quote($type_description[$i])."', 
												color_id = '$color_id[$i]', 
