...
<form name="product-detail" method="post" enctype="multipart/form-data">
	...
	if(empty($_POST['btn-product-detail'])){
      ...
      <div class="box row">
        ...
        <div class="content col-xs-9">
          ...
          <ul class="form-set" id="type_group_<?php echo $i;?>">
                ...
                <div class="">                  		
                  <div class="color-thumb edit image" style="padding-left:0;" onclick="openBrowser('color-image-<?=$i;?>')" onmouseover="imageOver('color-image-<?=$i;?>')" onMouseOut="imageOut('color-image-<?=$i;?>')">
                    <div class="content"> 
					  ...
					  <?php if($data['type_image'][$i]!=''){?>
                      
                      <img width="40" height="25" class="" id="upload-image-color-image-<?=$i;?>" src="<?php echo 'http://'.$_SERVER['HTTP_HOST'].get_dirname($_SERVER['PHP_SELF']).'/'.$prefix.$data['type_image'][$i];?>">
                      <input type="hidden" name="color_image_<?php echo $i;?>" value="<?php echo $data['type_image'][$i];?>">
            ...
            <li class="form-group row image-placeholder input-file"  id="type_image_field_<?=$i;?>">
