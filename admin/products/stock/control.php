...
$full_product = get_full_product($search_query, $cat, $sort_by, $query_per_page);
$total_query = $full_product['total_query'];
$total_page  = ceil($full_product['total_query'] / $query_per_page);
						

$all_product = get_all_product($search_query, $cat, $sort_by, $first_record, $query_per_page);
...
}else if($_REQUEST['srt'] == "stock_name DESC"){
   $arr_stock_name = "<span class=\"sort-arrow-down\"></span>";
}else if($_REQUEST['srt'] == "stock_name"){
   $arr_stock_name = "<span class=\"sort-arrow-up\"></span>";
...
// STORED VALUE
echo "<input type=\"hidden\" name=\"url\" id=\"url\" class=\"hidden\" value=\"http://".$_SERVER['HTTP_HOST'].get_dirname($_SERVER['PHP_SELF'])."/stock-view\">\n";
...
if(isset($_POST['btn_index_stock'])){
...
$type = $_POST['prod_id'];
   ...
   if($_POST['stock_action'] == "delete"){
	  ...
	  $_SESSION['alert'] = "success";
	  $_SESSION['msg']   = $count." item(s) has been successfully deleted ";
