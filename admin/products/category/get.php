...
function count_sub_category($parent_id, $relation){
   $conn   = connDB();
   $sql    = "SELECT COUNT(*) AS rows FROM `tbl_category_relation` WHERE category_parent = '$parent_id' AND `relation_level` = '$relation'";
   $query  = mysql_query($sql, $conn);
   $result = mysql_fetch_array($query);
